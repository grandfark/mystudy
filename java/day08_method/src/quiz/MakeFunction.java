...
public class MakeFunction {
	...
	public static void main(String[] args) {
		int INWON = 3;
		int ascr[][] = new int [INWON][3];
		String name[] = new String [INWON];
		
		input(name, ascr,INWON);
		
		int[] total = new int[INWON];
		
	}
		
	private static int output(int[] total , int INWON) {	
	...
	private static void input(String name[],int ascr[][], int INWON) {
		Scanner sc = new Scanner(System.in);
		for (int i = 0; i < INWON; i++) {
			System.out.print("이름 입력:");
			name[i] = sc.next();
			System.out.println("성적을 입력하세요. 국어, 영어, 전산");
			for(int j = 0; j < ascr[i].length; j++) {
				ascr[i][j]= sc.nextInt();
			}	
	}
	...
	/*	
	 	//성적처리 프로그램을 함수로 분리한다
반성공	//입력함수 input() - 이름 국어 영어 수학 점수
		//출력함수 output()
		//총점,평균 구하는 함수 total_avg
		//학점 구하는 함수 grade 
		//메인함수 버튼을 누르면 메뉴를 생성하는 방식으로 만들기
